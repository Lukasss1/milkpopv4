...
import {
  ...
  Receipt, Percent, Database, CloudUpload, CloudDownload, Megaphone, Send, Download
} from 'lucide-react';
import {
  ...
  Order, Deal, SiteSettings, ChecklistTemplateItem, CloudStatus, ClockHistoryItem, Payslip, EmailSettings
} from '../types';
...
import { sendEmail, payslipEmailHtml, newShiftEmailHtml, genericEmailHtml } from '../lib/notify';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  clockHistory, setClockHistory, payslips, setPayslips, emailSettings, setEmailSettings
}) => {
  ...
  // Payslip period selector — defaults to the previous calendar month
  const defaultPayslipMonth = (() => {
    const d = new Date(); d.setMonth(d.getMonth() - 1);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
  })();
  const [payslipMonth, setPayslipMonth] = useState<string>(defaultPayslipMonth);
  ...
  /** Salary contracts are converted to an effective hourly rate for timesheet pay. */
  const effectiveHourlyRate = (emp: EmployeeProfile): number => {
    const baseRate = emp.payRate || 11.44;
    if (emp.payType === 'salary' || baseRate >= 500) {
      return baseRate >= 20000 ? baseRate / 52 / 40 : (baseRate * 12) / 52 / 40;
    }
    return baseRate;
  };
  ...
  /** Approved (non-rejected) hours for an employee inside a YYYY-MM period. */
  const approvedHoursFor = (empId: string, periodKey: string) =>
    clockHistory
      .filter(ch => ch.employeeId === empId && ch.date.startsWith(periodKey) && ch.approved && !ch.rejected)
      .reduce((acc, ch) => acc + (ch.totalDecimalHours || 0), 0);

  const handleApproveTimesheet = (id: string) => {
    setClockHistory(prev => prev.map(ch => ch.id === id
      ? { ...ch, approved: true, rejected: false, approvedBy: employee?.name || 'Manager', approvedAt: new Date().toISOString() }
      : ch));
    logAction('Timesheets', `Approved timesheet entry ${id}`);
    addToast('Hours approved — they now count towards payroll.', 'success');
  };

  const handleRejectTimesheet = (id: string) => {
    setClockHistory(prev => prev.map(ch => ch.id === id
      ? { ...ch, approved: false, rejected: true, approvedBy: employee?.name || 'Manager', approvedAt: new Date().toISOString() }
      : ch));
    logAction('Timesheets', `Rejected timesheet entry ${id}`);
    addToast('Hours rejected — the team member will see this on their dashboard.', 'warning');
  };
  ...
  const notifyShiftByEmail = (shift: WorkShift) => {
    ...
    sendEmail({
      to: emp.email,
      subject: `New shift scheduled — ${new Date(shift.date + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}`,
      html: newShiftEmailHtml(shift, siteSettings.brandName || 'Milk Pop'),
  ...
  const sidebarSectionsAll = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'timesheets', label: 'Timesheet Approvals', icon: Clock, badge: clockHistory.filter(ch => !ch.approved && !ch.rejected).length, allowedRoles: ['owner', 'store_manager'] },
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center space-x-3 bg-[#EBDECE]/40 p-1.5 rounded-full pl-3">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-[#7CC0C7] text-[#2E2A26] hover:bg-[#5FA9B1] rounded-full text-[10px] uppercase font-bold transition-all cursor-pointer whitespace-nowrap"
          >
            Staff dashboard
          </button>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-4 text-2xs md:grid md:grid-cols-2 md:gap-4 md:space-y-0 text-[#2E2A26]">
                  ...
                  {formType === 'vacancy' && (
                    <>
                      ...
                      <div className="space-y-1.5">
                        <label className="font-bold">Salary (shown on the site)</label>
                        <input type="text" value={vacancyFormState.salary || ''} onChange={(e) => setVacancyFormState({ ...vacancyFormState, salary: e.target.value })} className="w-full bg-white border p-2 rounded-lg" placeholder="e.g. £12.50 / hr" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="font-bold">Contract type</label>
                        <select value={vacancyFormState.type || 'Part-time'} onChange={(e) => setVacancyFormState({ ...vacancyFormState, type: e.target.value as CareerVacancy['type'] })} className="w-full bg-white border p-2 rounded-lg">
                          <option value="Part-time">Part-time</option>
                          <option value="Full-time">Full-time</option>
                        </select>
                      </div>
                      <div className="space-y-1.5 md:col-span-2">
                        <label className="font-bold">Role description (shown to applicants) *</label>
                        <textarea rows={4} value={vacancyFormState.roleDescription || ''} onChange={(e) => setVacancyFormState({ ...vacancyFormState, roleDescription: e.target.value })} className="w-full bg-white border p-2 rounded-lg" placeholder="Describe the role, the vibe, and what a typical shift looks like…" />
                      </div>
                      <div className="space-y-1.5 md:col-span-2">
                        <label className="font-bold">Requirements <span className="font-normal text-stone-400">(one per line)</span></label>
                        <textarea rows={3} value={(vacancyFormState.requirements || []).join('\n')} onChange={(e) => setVacancyFormState({ ...vacancyFormState, requirements: e.target.value.split('\n').filter(l => l.trim() !== '') })} className="w-full bg-white border p-2 rounded-lg font-mono text-[11px]" placeholder={'Right to work in the UK\nWeekend availability'} />
                      </div>
                      <div className="space-y-1.5 md:col-span-2">
                        <label className="font-bold">Responsibilities <span className="font-normal text-stone-400">(one per line)</span></label>
                        <textarea rows={3} value={(vacancyFormState.responsibilities || []).join('\n')} onChange={(e) => setVacancyFormState({ ...vacancyFormState, responsibilities: e.target.value.split('\n').filter(l => l.trim() !== '') })} className="w-full bg-white border p-2 rounded-lg font-mono text-[11px]" placeholder={'Craft signature milkshakes\nKeep counters spotless'} />
                      </div>
                    </>
                  )}
                  ...
                  <button
                    onClick={() => {
                      if (formType === 'menu') {
                        ...
                        addToast(`Associate "${newStaff.name}" onboarded${cloudStatus.configured ? ' and synced to the cloud database' : ''}.`, 'success');
                      } else if (formType === 'vacancy') {
                        if (!vacancyFormState.title?.trim()) {
                          addToast('Please give the vacancy a role name.', 'error');
                          return;
                        }
                        if (editItemId) {
                          ...
                          const newVac: CareerVacancy = {
                            id: nextId,
                            title: vacancyFormState.title.trim(),
                            ...
                            type: vacancyFormState.type || 'Part-time',
                            roleDescription: vacancyFormState.roleDescription || '',
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="p-3.5 bg-[#EBDECE]/15 border border-[#EBDECE]/40 rounded-xl space-y-1">
                          ...
                          <button onClick={() => {
                            const updated = { ...selectedStaffUser, holidayBalance: (selectedStaffUser.holidayBalance || 0) + 1.5 };
                            onUpdateEmployee(updated);
                            setSelectedStaffUser(updated);
                            logAction('Staff HR Directory', `Added 1.5 holiday days for "${updated.name}"`);
                            addToast('Holiday allowance updated and saved.', 'success');
                          }} className="text-[9px] hover:underline block text-stone-500 font-bold cursor-pointer">Add Allowance</button>
                        </div>
                           ...
                           <div>
                             ...
                             <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                               {(() => {
                                  ...
                                  if (userClocks.length === 0) return <div className="text-center text-neutral-400 italic py-2">No timesheet records yet.</div>;
          ...
          {activeTab === 'rota' && (
            <div className="space-y-6">
              ...
              {(() => {
                ...
                const base = new Date();
                const dayIdx = (base.getDay() + 6) % 7; // Mon=0 … Sun=6
                base.setHours(0, 0, 0, 0);
                base.setDate(base.getDate() - dayIdx + weekOffset * 7);
                ...
                const weekShifts = shiftsList.filter(s => weekIsos.includes(s.date));
                const hoursOf = (s: WorkShift) => {
                ...
                const rangeLabel = `${weekDays[0].toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} – ${weekDays[6].toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`;
                  ...
                  <div className="bg-white rounded-2xl border shadow-sm p-5 space-y-4">
                    <div className="flex items-center justify-between flex-wrap gap-3 border-b pb-3">
                      <h3 className="font-display font-black uppercase tracking-wider text-xs flex items-center gap-2">
                        <Users className="h-4 w-4 text-[#BD783A]" />
                        Whole-Team Week View
                      </h3>
                      <div className="flex items-center gap-2">
                        <button onClick={() => setWeekOffset(w => w - 1)} className="p-1.5 border rounded-lg hover:bg-stone-50 cursor-pointer" aria-label="Previous week">
                          <ChevronLeft className="h-4 w-4" />
                        </button>
                        <span className="text-2xs font-black min-w-[170px] text-center">
                          {rangeLabel}
                          {weekOffset === 0 && <span className="ml-1.5 text-[8px] bg-[#7CC0C7]/30 text-[#2E2A26] px-1.5 py-0.5 rounded-full uppercase">This week</span>}
                        </span>
                        <button onClick={() => setWeekOffset(w => w + 1)} className="p-1.5 border rounded-lg hover:bg-stone-50 cursor-pointer" aria-label="Next week">
                          <ChevronRight className="h-4 w-4" />
                        </button>
                        {weekOffset !== 0 && (
                          <button onClick={() => setWeekOffset(0)} className="px-2.5 py-1.5 border rounded-lg text-[9px] font-black uppercase hover:bg-stone-50 cursor-pointer">Today</button>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse min-w-[860px] text-2xs">
                        <thead>
                          <tr className="text-[9px] uppercase font-black text-stone-500">
                            <th className="p-2 text-left w-44 bg-stone-50 border border-[#EBDECE]/60 sticky left-0">Associate</th>
                            {weekDays.map((d, i) => (
                              <th key={i} className={`p-2 border border-[#EBDECE]/60 text-center ${iso(d) === todayIso ? 'bg-[#7CC0C7]/20' : 'bg-stone-50'}`}>
                                {d.toLocaleDateString('en-GB', { weekday: 'short' })}
                                <span className="block font-mono text-[8px] text-stone-400">{d.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
                              </th>
                            ))}
                            <th className="p-2 border border-[#EBDECE]/60 bg-stone-50 text-right w-16">Hours</th>
                          </tr>
                        </thead>
                        <tbody>
                          {employeesList.map(emp => {
                            const empShifts = weekShifts.filter(s => s.employeeId === emp.id);
                            const totalHrs = empShifts.reduce((a, s) => a + hoursOf(s), 0);
                            return (
                              <tr key={emp.id} className="hover:bg-amber-50/20">
                                <td className="p-2 border border-[#EBDECE]/60 bg-white sticky left-0">
                                  <p className="font-extrabold text-[#2E2A26] uppercase text-[10px] leading-tight">{emp.name}</p>
                                  <p className="font-mono text-[8px] text-stone-400">{emp.role.replace('_', ' ')}</p>
                                </td>
                                {weekIsos.map(dayIso => {
                                  const cell = empShifts.filter(s => s.date === dayIso).sort((a, b) => a.startTime.localeCompare(b.startTime));
                                  return (
                                    <td key={dayIso} className={`p-1.5 border border-[#EBDECE]/60 align-top text-center ${dayIso === todayIso ? 'bg-[#7CC0C7]/10' : ''}`}>
                                      {cell.length === 0 ? (
                                        <span className="text-stone-200 select-none">—</span>
                                      ) : cell.map(s => (
                                        <div key={s.id} className={`group relative mb-1 last:mb-0 rounded-lg px-1.5 py-1 text-[9px] font-mono font-bold border ${
                                          s.type === 'opening' ? 'bg-amber-50 border-amber-200 text-amber-800'
                                          : s.type === 'closing' ? 'bg-indigo-50 border-indigo-200 text-indigo-800'
                                          : s.type === 'training' ? 'bg-emerald-50 border-emerald-200 text-emerald-800'
                                          : 'bg-[#EBDECE]/40 border-[#D2C5B4] text-[#2E2A26]'
                                        }`}>
                                          {s.startTime}–{s.endTime}
                                          <span className="block text-[7px] uppercase font-black opacity-60">{s.type}</span>
                                          <button
                                            type="button"
                                            onClick={() => { onDeleteShift(s.id); logAction('Schedulers', `Deleted shift via week grid (${emp.name} ${s.date})`); addToast('Shift removed.', 'warning'); }}
                                ...
                                <td className="p-2 border border-[#EBDECE]/60 text-right font-mono font-bold text-[10px]">
                                  {totalHrs > 0 ? `${totalHrs.toFixed(1)}h` : <span className="text-stone-300">0</span>}
                                </td>
                              </tr>
                            );
                          })}
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-2xs text-[#2E2A26]/70">Every clock-out lands here as <b>Pending</b>. Only hours you approve count towards payslips.</p>
                </div>
                {clockHistory.filter(ch => !ch.approved && !ch.rejected).length > 1 && (
                  <button
                    onClick={() => {
                      const pendingCount = clockHistory.filter(ch => !ch.approved && !ch.rejected).length;
                      if (!window.confirm(`Approve all ${pendingCount} pending entries?`)) return;
                      const stamp = new Date().toISOString();
                      setClockHistory(prev => prev.map(ch => (!ch.approved && !ch.rejected)
                        ? { ...ch, approved: true, approvedBy: employee?.name || 'Manager', approvedAt: stamp }
                        : ch));
                      logAction('Timesheets', `Bulk-approved ${pendingCount} timesheet entries`);
                      addToast(`${pendingCount} timesheet entries approved.`, 'success');
                    }}
                    className="px-4 py-2 bg-[#5CA459] text-white rounded-full text-2xs tracking-wider uppercase font-black cursor-pointer hover:bg-[#4E8E4B]"
                  >
                    Approve all pending
                  </button>
                )}
              ...
              {(['pending', 'processed'] as const).map(section => {
                const rows = clockHistory
                  .filter(ch => section === 'pending' ? (!ch.approved && !ch.rejected) : (ch.approved || ch.rejected))
                  .sort((a, b) => (b.clockIn || '').localeCompare(a.clockIn || ''));
                return (
                  <div key={section} className="bg-white rounded-2xl border border-[#EBDECE]/50 overflow-hidden shadow-2xs">
                    <div className="px-4 py-3 bg-[#DFD3C3]/40 border-b flex items-center justify-between">
                      <h3 className="font-display font-black text-xs uppercase tracking-wide">
                        {section === 'pending' ? 'Awaiting approval' : 'Processed history'}
                      </h3>
                      <span className="text-[9px] font-mono bg-white border px-2 py-0.5 rounded">{rows.length} entries</span>
                    </div>
                    {rows.length === 0 ? (
                      <p className="text-center py-8 text-stone-400 font-mono text-[10px]">
                        {section === 'pending' ? 'Nothing waiting — all caught up. 🥛' : 'No processed entries yet.'}
                      </p>
                    ) : (
                      <table className="w-full text-left text-2xs">
                        <thead className="border-b text-[9px] uppercase font-mono text-stone-500">
                          <tr>
                            ...
                            <th className="p-3 text-right">{section === 'pending' ? 'Decision' : 'Status'}</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y text-stone-600">
                          {rows.slice(0, section === 'processed' ? 40 : undefined).map(ch => {
                            const emp = employeesList.find(e => e.id === ch.employeeId);
                            const rate = emp ? effectiveHourlyRate(emp) : 11.44;
                            return (
                              <tr key={ch.id} className="hover:bg-amber-50/20">
                                <td className="p-3 font-bold text-[#2E2A26]">
                                  {ch.employeeName}
                                  {ch.notes && <span className="block text-[9px] font-normal text-stone-400 italic">“{ch.notes}”</span>}
                                </td>
                                <td className="p-3 font-mono text-[10px]">{new Date(ch.date + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</td>
                                <td className="p-3 font-mono text-[10px]">
                                  {new Date(ch.clockIn).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })} → {ch.clockOut ? new Date(ch.clockOut).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) : '—'}
                                </td>
                                <td className="p-3 font-mono text-[10px]">{ch.breakDurationMinutes || 0} min</td>
                                <td className="p-3 text-right font-mono font-bold">{(ch.totalDecimalHours || 0).toFixed(2)}</td>
                                <td className="p-3 text-right font-mono">£{((ch.totalDecimalHours || 0) * rate).toFixed(2)}</td>
                                <td className="p-3 text-right">
                                  {section === 'pending' ? (
                                    <div className="flex justify-end gap-1.5">
                                      <button onClick={() => handleApproveTimesheet(ch.id)} className="px-2.5 py-1 bg-[#5CA459] hover:bg-[#4E8E4B] text-white rounded font-black text-[9px] uppercase cursor-pointer">Approve</button>
                                      <button onClick={() => handleRejectTimesheet(ch.id)} className="px-2.5 py-1 border border-red-300 text-red-500 hover:bg-red-50 rounded font-black text-[9px] uppercase cursor-pointer">Reject</button>
                                    </div>
                                  ) : ch.rejected ? (
                                    <span className="text-[9px] font-black uppercase text-red-500">Rejected · {ch.approvedBy}</span>
                                  ) : (
                                    <span className="text-[9px] font-black uppercase text-emerald-600">Approved · {ch.approvedBy}</span>
                                  )}
                                </td>
                              </tr>
                            );
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-2xs text-[#2E2A26]/70">Payslips are built from <b>approved</b> timesheet hours only. Generate a month, review it, then e-mail each statement.</p>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="month"
                    ...
                    className="bg-white border border-[#EBDECE] rounded-full px-4 py-2 text-2xs font-mono outline-none focus:border-[#BD783A]"
                  />
                  <button
                    onClick={() => {
                      const label = monthLabel(payslipMonth);
                      const created: Payslip[] = [];
                      let skippedExisting = 0, skippedNoHours = 0;
                      employeesList.forEach(emp => {
                        if (payslips.some(p => p.employeeId === emp.id && p.periodKey === payslipMonth)) { skippedExisting++; return; }
                        const hrs = approvedHoursFor(emp.id, payslipMonth);
                        ...
                        created.push({
                          ...
                          generatedBy: employee?.name || 'Administrator'
                        });
                      });
                      if (created.length === 0) {
                        addToast(
                          skippedExisting && !skippedNoHours
                            ? `Payslips for ${label} already exist for everyone with hours.`
                      ...
                      logAction('Payslips Ledger', `Generated ${created.length} payslips for ${label}`);
                      addToast(`${created.length} payslip${created.length > 1 ? 's' : ''} generated for ${label}${skippedNoHours ? ` (${skippedNoHours} staff had no approved hours)` : ''}. Now review and send.`, 'success');
                    }}
                    className="px-4 py-2 bg-[#BD783A] text-white rounded-full text-2xs tracking-wider uppercase font-black cursor-pointer hover:bg-[#A5642B]"
                  >
                    Generate {monthLabel(payslipMonth)}
                  </button>
                </div>
              </div>

              {!emailSettings.enabled && (
                <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-2xs text-amber-800">
                  E-mail dispatch is switched off. Turn it on in <b>Company Settings → E-mail notifications</b> (a one-off 5-minute setup) to send payslips to staff inboxes.
                </div>
              )}

              {/* Preview: approved hours in the selected month */}
              <div className="bg-white rounded-2xl border border-[#EBDECE]/50 overflow-hidden shadow-2xs">
                <div className="px-4 py-3 bg-[#DFD3C3]/40 border-b">
                  <h3 className="font-display font-black text-xs uppercase tracking-wide">Approved hours — {monthLabel(payslipMonth)}</h3>
                </div>
                <table className="w-full text-left text-2xs">
                  <thead className="border-b text-[10px] uppercase font-mono text-[#2E2A26]">
                    <tr>
                      <th className="p-4">Employee</th>
                      <th className="p-4">Rate (hr)</th>
                      <th className="p-4">Approved hours</th>
                      <th className="p-4">Pending hours</th>
                      <th className="p-4">Gross (approved)</th>
                  ...
                  <tbody className="divide-y text-stone-600 font-medium">
                    {employeesList.map(emp => {
                      const rate = effectiveHourlyRate(emp);
                      const approvedHrs = approvedHoursFor(emp.id, payslipMonth);
                      const pendingHrs = clockHistory
                        .filter(ch => ch.employeeId === emp.id && ch.date.startsWith(payslipMonth) && !ch.approved && !ch.rejected)
                        .reduce((a, c) => a + (c.totalDecimalHours || 0), 0);
                      const entries = clockHistory
                        .filter(ch => ch.employeeId === emp.id && ch.date.startsWith(payslipMonth) && ch.approved && !ch.rejected)
                      ...
                      return (
                        <React.Fragment key={emp.id}>
                          <tr className="hover:bg-amber-50/20">
                            ...
                            <td className="p-4 font-mono">£{rate.toFixed(2)}{(emp.payType === 'salary' || (emp.payRate || 0) >= 500) && <span className="block text-[9px] text-stone-400 font-normal">(salary equivalent)</span>}</td>
                            ...
                            <td className="p-4 font-mono font-bold text-[#5CA459]">£{(approvedHrs * rate).toFixed(2)}</td>
                            <td className="p-4">
                              <button
                                onClick={() => setExpandedPayslipEmp(isOpen ? null : emp.id)}
                                disabled={entries.length === 0}
                                className="text-[#BD783A] hover:underline cursor-pointer disabled:text-stone-300 disabled:no-underline disabled:cursor-default"
                              >
                          ...
                          {isOpen && entries.length > 0 && (
                            <tr>
                              <td colSpan={6} className="bg-stone-50 p-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                  {entries.map(ch => (
                                    <div key={ch.id} className="bg-white border rounded-xl p-2.5 text-[10px] font-mono flex justify-between">
                                      <span>{new Date(ch.date + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
                                      <span className="text-stone-400">{new Date(ch.clockIn).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}–{ch.clockOut ? new Date(ch.clockOut).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) : '—'}</span>
              ...
              <div className="bg-white rounded-2xl border border-[#EBDECE]/50 overflow-hidden shadow-2xs">
                <div className="px-4 py-3 bg-[#DFD3C3]/40 border-b flex items-center justify-between">
                  <h3 className="font-display font-black text-xs uppercase tracking-wide">Issued payslips</h3>
                  <span className="text-[9px] font-mono bg-white border px-2 py-0.5 rounded">{payslips.length} total</span>
                </div>
                {payslips.length === 0 ? (
                  <p className="text-center py-8 text-stone-400 font-mono text-[10px]">No payslips generated yet — pick a month above and press Generate.</p>
                ) : (
                  <table className="w-full text-left text-2xs">
                    <thead className="border-b text-[10px] uppercase font-mono text-[#2E2A26]">
                      <tr>
                    ...
                    <tbody className="divide-y text-stone-600 font-medium">
                      {[...payslips].sort((a, b) => b.periodKey.localeCompare(a.periodKey) || a.employeeName.localeCompare(b.employeeName)).map(p => (
                        <tr key={p.id} className="hover:bg-amber-50/20">
                          <td className="p-4 font-bold text-[#2E2A26]">{p.employeeName}<span className="block text-[9px] font-mono font-normal text-stone-400">{p.email}</span></td>
                          <td className="p-4">{p.periodLabel}</td>
                          <td className="p-4 font-mono">{p.hoursTotal.toFixed(1)} hrs @ £{p.hourlyRate.toFixed(2)}</td>
                          <td className="p-4 font-mono font-bold text-[#5CA459]">£{p.net.toFixed(2)}</td>
                          <td className="p-4">
                            {p.status === 'sent'
                              ? <span className="text-[9px] font-black uppercase bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">Sent {p.sentAt ? new Date(p.sentAt).toLocaleDateString('en-GB') : ''}</span>
                              : <span className="text-[9px] font-black uppercase bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">Draft</span>}
                          </td>
                          <td className="p-4 text-right space-x-2 whitespace-nowrap">
                            <button
                              disabled={emailBusyId === p.id}
                              onClick={async () => {
                                if (!emailSettings.enabled) { addToast('Enable e-mail notifications in Company Settings first.', 'warning'); return; }
                                setEmailBusyId(p.id);
                                const err = await sendEmail({
                                  to: p.email,
                                  subject: `Your ${p.periodLabel} payslip is available`,
                                  html: payslipEmailHtml(p, siteSettings.currencySymbol || '£', siteSettings.brandName || 'Milk Pop'),
                                  fromName: emailSettings.fromName
                                });
                                setEmailBusyId(null);
                                if (err) {
                                  addToast('Payslip e-mail failed: ' + err, 'error');
                                } else {
                                  setPayslips(prev => prev.map(x => x.id === p.id ? { ...x, status: 'sent', sentAt: new Date().toISOString() } : x));
                                  logAction('Payslips Ledger', `Emailed ${p.periodLabel} payslip to ${p.employeeName} (${p.email})`);
                                  addToast(`Payslip e-mailed to ${p.employeeName}.`, 'success');
                                }
                              }}
                              className="text-[#BD783A] hover:underline cursor-pointer inline-flex items-center gap-1 disabled:opacity-50"
                            >
                            ...
                            <button
                              onClick={() => {
                                if (!window.confirm(`Delete the ${p.periodLabel} payslip for ${p.employeeName}?`)) return;
                                setPayslips(prev => prev.filter(x => x.id !== p.id));
                                logAction('Payslips Ledger', `Deleted ${p.periodLabel} payslip for ${p.employeeName}`);
                                addToast('Payslip deleted.', 'warning');
                              }}
          ...
          {activeTab === 'careers' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => {
                    setFormType('vacancy');
                    setEditItemId(null);
                    setVacancyFormState({ title: '', department: 'Store Management Operations', location: 'Solihull', salary: '£12.50 / hr', type: 'Part-time', roleDescription: '', requirements: [], responsibilities: [] });
                    setIsFormOpen(true);
                  }}
              ...
              {/* Live vacancies — everything here is editable, including the description shown on the public Careers page */}
              <div className="space-y-4">
                <h3 className="font-display font-black text-xs uppercase tracking-wide">Live Vacancies ({vacancies.length})</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {vacancies.map(vac => (
                    <div key={vac.id} className="p-4 bg-white rounded-2xl border border-[#EBDECE]/50 space-y-2">
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <h4 className="font-extrabold text-sm text-[#2E2A26]">{vac.title}</h4>
                          <p className="text-[9px] font-mono text-stone-400 uppercase">{vac.department} · {vac.location} · {vac.type} · {vac.salary}</p>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            onClick={() => {
                              setFormType('vacancy');
                              setEditItemId(vac.id);
                              setVacancyFormState({ ...vac });
                              setIsFormOpen(true);
                            }}
                            className="p-1.5 border rounded-lg text-[#BD783A] hover:bg-[#EBDECE]/30 cursor-pointer"
                          ...
                          <button
                            onClick={() => {
                              if (!window.confirm(`Remove the "${vac.title}" vacancy from the public site?`)) return;
                              setVacancies(vacancies.filter(v => v.id !== vac.id));
                              logAction('Careers Recruiter', `Closed vacancy "${vac.title}"`);
                              addToast(`Vacancy "${vac.title}" removed from the site.`, 'warning');
                            }}
                            className="p-1.5 border rounded-lg text-red-500 hover:bg-red-50 cursor-pointer"
                      ...
                      <p className="text-[11px] text-[#2E2A26]/75 leading-relaxed line-clamp-3">{vac.roleDescription || <span className="italic text-stone-400">No description yet — press edit to write one.</span>}</p>
                    </div>
                  ))}
                  {vacancies.length === 0 && (
                    <p className="text-[10px] font-mono text-stone-400 col-span-2 text-center py-6">No live vacancies — create one to accept applications.</p>
                  )}
                </div>
              </div>
                    ...
                    <div key={app.id} className="p-4 bg-white rounded-2xl border border-[#EBDECE]/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="space-y-1.5 flex-1">
                        ...
                        <p className="text-zinc-500 font-medium">
                          Applied for: <b>{app.position}</b> | Experience: "{app.experience}" | CV:{' '}
                          {app.cvUrl ? (
                            <a href={app.cvUrl} target="_blank" rel="noopener noreferrer" className="text-sky-700 underline font-mono cursor-pointer inline-flex items-center gap-1">
                              {app.cvName || 'Open CV'} <Download className="h-3 w-3" />
                            </a>
                          ) : app.cvData ? (
                            <a href={app.cvData} download={app.cvName || 'cv'} className="text-sky-700 underline font-mono cursor-pointer inline-flex items-center gap-1">
                              {app.cvName || 'Download CV'} <Download className="h-3 w-3" />
                            </a>
                          ) : (
                            <span className="text-stone-400 font-mono italic">not attached</span>
                          )}
                        </p>
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => { onUpdateApplicationStatus(app.id, 'reviewing'); logAction('Careers Desk', `Moved candidacy status of "${app.fullName}" to screening`); addToast(`Candidate "${app.fullName}" moved to screening.`, 'info'); }} className="p-1 border hover:bg-stone-55 rounded text-2xs font-bold font-mono">SCREEN</button>
                          <button onClick={() => {
                            onUpdateApplicationStatus(app.id, 'interview');
                            logAction('Careers Desk', `Marked "${app.fullName}" for interview`);
                            if (emailSettings.enabled) {
                              sendEmail({
                                to: app.email,
                                subject: `Your ${siteSettings.brandName || 'Milk Pop'} application — interview invitation`,
                                html: genericEmailHtml('Interview invitation', `Hi ${app.fullName.split(' ')[0]}, thank you for applying for the <b>${app.position}</b> role. We'd love to invite you to an interview — we'll be in touch shortly to arrange a time.`, siteSettings.brandName || 'Milk Pop'),
                                fromName: emailSettings.fromName
                              }).then(err => addToast(err ? 'Status updated, but the e-mail failed: ' + err : `Interview e-mail sent to ${app.fullName}.`, err ? 'warning' : 'success'));
                            } else {
                              addToast(`"${app.fullName}" marked for interview. (Enable e-mail in Company Settings to notify candidates automatically.)`, 'info');
                            }
                          }} className="p-1 border hover:bg-stone-55 rounded text-2xs font-bold font-mono">INTERVIEW</button>
                          <button onClick={() => {
                            onUpdateApplicationStatus(app.id, 'offer');
                            logAction('Careers Desk', `Extended offer to candidate "${app.fullName}"`);
                            if (emailSettings.enabled) {
                              sendEmail({
                                to: app.email,
                                subject: `Great news about your ${siteSettings.brandName || 'Milk Pop'} application! 🥛`,
                                html: genericEmailHtml('You have an offer!', `Hi ${app.fullName.split(' ')[0]}, we're delighted to offer you the <b>${app.position}</b> position. We'll follow up with your contract and start details very soon. Welcome to the team!`, siteSettings.brandName || 'Milk Pop'),
                                fromName: emailSettings.fromName
                              }).then(err => addToast(err ? 'Offer recorded, but the e-mail failed: ' + err : `Offer e-mail sent to ${app.fullName}!`, err ? 'warning' : 'success'));
                            } else {
                              addToast(`Offer recorded for "${app.fullName}". (Enable e-mail in Company Settings to notify candidates automatically.)`, 'success');
                            }
                          }} className="p-1.5 bg-[#5CA459] hover:bg-[#4E8E4B] text-white rounded font-black font-mono">HIRE</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-[#EBDECE] p-6 space-y-4 shadow-2xs font-sans text-2xs text-[#2E2A26]">
                <h3 className="font-display font-black text-xs uppercase tracking-wide border-b border-[#EBDECE] pb-2 flex items-center gap-2"><Mail className="h-3.5 w-3.5 text-[#BD783A]" /> E-mail notifications (payslips, new shifts, candidates)</h3>
                <p className="text-[#2E2A26]/70 leading-relaxed">
                  E-mails are sent server-side through the Supabase Edge Function <span className="font-mono">send-email</span> (a browser can't send mail directly without exposing your credentials).
                  One-off setup takes ~5 minutes: create a free <b>Resend</b> account, then deploy the function — full steps in <span className="font-mono">supabase/EMAIL_SETUP.md</span>.
                </p>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input type="checkbox" checked={emailSettings.enabled} onChange={e => setEmailSettings(s => ({ ...s, enabled: e.target.checked }))} className="h-4 w-4 rounded border-neutral-300" />
                  <span className="font-bold">Enable e-mail sending</span>
                </label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1"><label className="font-bold block">Sender name</label>
                    <input value={emailSettings.fromName} onChange={e => setEmailSettings(s => ({ ...s, fromName: e.target.value }))} className="w-full bg-stone-50 border border-[#EBDECE] p-2.5 rounded-xl outline-none focus:border-[#BD783A]" placeholder="Milk Pop" /></div>
                  <label className="flex items-end gap-2 cursor-pointer select-none pb-2.5">
                    <input type="checkbox" checked={emailSettings.notifyNewShift} onChange={e => setEmailSettings(s => ({ ...s, notifyNewShift: e.target.checked }))} className="h-4 w-4 rounded border-neutral-300" />
                    <span className="font-bold">"New shift scheduled" e-mails</span>
                  </label>
                  <label className="flex items-end gap-2 cursor-pointer select-none pb-2.5">
                    <input type="checkbox" checked={emailSettings.notifyPayslip} onChange={e => setEmailSettings(s => ({ ...s, notifyPayslip: e.target.checked }))} className="h-4 w-4 rounded border-neutral-300" />
                    <span className="font-bold">Payslip e-mails</span>
                  </label>
                </div>
                <div className="pt-2 border-t border-[#EBDECE] flex items-center gap-3 flex-wrap">
                  <button
                    disabled={emailBusyId === '__test__'}
                    onClick={async () => {
                      const to = window.prompt('Send a test e-mail to which address?', employee?.email || '');
                      if (!to) return;
                      setEmailBusyId('__test__');
                      const err = await sendEmail({
                        to,
                        subject: `${siteSettings.brandName || 'Milk Pop'} — test e-mail ✅`,
                        html: genericEmailHtml('It works!', 'This is a test message from your staff platform. Payslip and shift notifications will arrive exactly like this.', siteSettings.brandName || 'Milk Pop'),
                        fromName: emailSettings.fromName
                      });
                      setEmailBusyId(null);
                      if (err) addToast('Test e-mail failed: ' + err, 'error');
                      else { logAction('Company Settings', `Sent test e-mail to ${to}`); addToast(`Test e-mail sent to ${to} — check the inbox (and spam folder).`, 'success'); }
                    }}
                    className="px-4 py-2.5 bg-[#2E2A26] hover:bg-[#BD783A] text-white rounded-full uppercase font-black tracking-wider cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                  >
                    <Send className="h-3.5 w-3.5" /> {emailBusyId === '__test__' ? 'Sending…' : 'Send test e-mail'}
                  </button>
                  <span className="text-[10px] text-[#2E2A26]/60">Preferences save automatically and sync to the cloud.</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-[#7CC0C7] p-6 space-y-4 shadow-2xs font-sans text-2xs text-[#2E2A26]">
                ...
                <p className="text-[#2E2A26]/70 leading-relaxed">Run <span className="font-mono">supabase/schema.sql</span> then <span className="font-mono">supabase/seed.sql</span> in your Supabase project's SQL editor (existing projects: run <span className="font-mono">supabase/migration_payroll_cv.sql</span> instead — it adds the timesheet, payslip and CV-storage pieces without touching your data), paste the Project URL and anon key below, and every registry — menu, orders, staff, timesheets, payslips, all of it — mirrors to the cloud automatically and loads back on any device.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
