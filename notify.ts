...
export default function App() {
  ...
  const [clockHistory, setClockHistory] = useLocalStorageState<ClockHistoryItem[]>('milkpop_clock_history', []);
  const [payslips, setPayslips] = useLocalStorageState<Payslip[]>('milkpop_payslips', []);

  // E-mail notification preferences (delivery via the Supabase Edge Function)
  const [emailSettings, setEmailSettings] = useLocalStorageState<EmailSettings>('milkpop_email_settings', DEFAULT_EMAIL_SETTINGS);
  ...
  useEffect(() => {
    ...
    const applyCloudData = (data: Record<string, any>) => {
      ...
      if (data.__kv__) {
        ...
        if (data.__kv__['milkpop_email_settings']) setEmailSettings(data.__kv__['milkpop_email_settings']);
      }
      apply('milkpop_clock_history', setClockHistory);
      apply('milkpop_payslips', setPayslips);
