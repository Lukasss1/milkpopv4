...
export interface JobApplication {
  ...
  experience: string;
  cvName: string;
  /** Public URL of the CV in Supabase Storage (bucket `cvs`) when cloud is connected. */
  cvUrl?: string;
  /** Base64 data-URL fallback used when no cloud storage is configured (small files only). */
  cvData?: string;
  message: string;
