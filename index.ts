...
/* ------------------------------------------------------------------ */
/*  SUPABASE STORAGE — file uploads (CVs, documents).                  */
/*  Files go into a public bucket; the returned URL is stored in the   */
/*  database row so the Admin Panel can open the file from any device. */
/* ------------------------------------------------------------------ */

/** Upload a File to a public Storage bucket. Returns the public URL. */
export async function sbUploadFile(bucket: string, path: string, file: File | Blob): Promise<string> {
  const cfg = getSupabaseConfig();
  if (!cfg) throw new Error('Supabase is not configured');
...
/** Make a filename safe for a Storage object key. */
