...
import { isCloudConfigured, sbUploadFile, sanitizeFileName } from '../lib/supabase';
import {
  Sparkles,
  MapPin,
  Clock,
...
export const PublicPages: React.FC<PublicPagesProps> = ({
  ...
  const handleCareerSubmit = async (e: React.FormEvent) => {
    ...
    // Attach the CV: Supabase Storage when the cloud is connected,
    // otherwise a base64 copy stored on the application record itself.
    ...
    if (cvFileObj) {
      ...
      try {
        if (isCloudConfigured()) {
          const path = `${Date.now()}_${sanitizeFileName(cvFileObj.name)}`;
          cvUrl = await sbUploadFile('cvs', path, cvFileObj);
        } else {
          if (cvFileObj.size > 2.5 * 1024 * 1024) {
            addToast('CV is too large to store locally (max 2.5 MB without a cloud connection). Please attach a smaller file.', 'error');
          ...
          cvData = await new Promise<string>((resolve, reject) => {
            ...
            reader.readAsDataURL(cvFileObj);
          });
        }
      } catch (err: any) {
        setCvUploading(false);
        addToast('CV upload failed: ' + (err?.message || err) + ' — please try again.', 'error');
        return;
      }
      setCvUploading(false);
    }
    ...
    const newApp: JobApplication = {
      ...
      cvName: cvFileObj ? cvFileObj.name : '',
      cvUrl,
      cvData,
    ...
    addToast(`Thank you ${careerForm.fullName}! Your application${cvFileObj ? ' and CV have' : ' has'} been received. Our recruitment manager will review it.`, 'success');
    setCareerForm({
      fullName: '',
      email: '',
      phone: '',
                ...
                <div className="space-y-6">
                  ...
                  <form onSubmit={handleCareerSubmit} className="space-y-4">
                    ...
                    <div className="bg-[#FFFFFF] border border-dashed border-[#BD783A]/40 p-4 rounded-xl space-y-2 text-center">
                      <p className="text-[10px] font-bold uppercase text-[#2E2A26]/85">Upload your CV (PDF or Word, max 5 MB)</p>
                      <input
                        ref={cvInputRef}
                        id="app-cv-upload"
                        type="file"
                        ...
                        onChange={(e) => {
                          ...
                          if (f.size > 5 * 1024 * 1024) {
                            addToast('That file is over 5 MB — please attach a smaller CV.', 'error');
                            e.target.value = '';
                            return;
                          }
                          setCvFileObj(f);
                      ...
                      <div className="flex justify-center items-center gap-2 flex-wrap">
                        ...
                        {cvFile ? (
                          <span className="text-[10px] text-[#5FA777] font-semibold inline-flex items-center gap-1.5">
                            {cvFile} ✅
                            <button type="button" onClick={() => { setCvFile(''); setCvFileObj(null); if (cvInputRef.current) cvInputRef.current.value = ''; }} className="text-red-400 hover:text-red-600 font-black cursor-pointer" aria-label="Remove file">✕</button>
                          </span>
                        ) : (
                          <span className="text-[10px] text-gray-400 font-light">No file selected (optional)</span>
                        )}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-[#BD783A] hover:bg-[#2E2A26] text-white rounded-full text-xs font-black uppercase tracking-wider transition-colors cursor-pointer disabled:opacity-60"
                    >
                      {cvUploading ? 'Uploading CV…' : 'Submit Career Application'}
                    </button>
                  </form>
                </div>
              ) : (
